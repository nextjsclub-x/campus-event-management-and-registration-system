...
export async function getParticipationAnalytics(activityId: number) {
	...
	const rates = {
		attendanceRate:
			participationStats.registered > 0
				? (
						(participationStats.attended / participationStats.registered) *
						100
					).toFixed(2)
				: '0.00',
		absentRate:
			participationStats.registered > 0
				? (
						(participationStats.absent / participationStats.registered) *
						100
					).toFixed(2)
				: '0.00',
		cancellationRate:
			participationStats.registered > 0
				? (
						(participationStats.cancelled / participationStats.registered) *
						100
					).toFixed(2)
				: '0.00',
	};
